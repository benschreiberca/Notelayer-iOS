...
| `Native_Parity_Map.md` | Describes a web app with Supabase backend that does not exist. Calls backend "Supabase" throughout — backend is Firebase. Describes 3 tabs. Aspirational document mislabeled as parity map. |
...
|------|---------|-------------|
| `Native_Parity_Map.md` | Entire web app + Supabase backend | ❌ No web app exists; Firebase is backend |
...
| **Web app** | `Native_Parity_Map.md` (maps features to a "web app"), `firebase-hosting/index.html` | `firebase-hosting/index.html` is boilerplate placeholder HTML — no JavaScript app | The web app described in parity docs does not exist. `firebase-hosting` has only an empty Firebase Hosting stub. |
...
| `Native_Parity_Map.md` | Entire document maps to a Supabase-backed web app | Web app doesn't exist. Backend is Firebase. |
