...
- [ ] 🟩 **Step 1: Baseline & Standards Alignment**
  - [ ] 🟩 Identify the Standard-Bearer file(s) for platform-consistent UI (use for comparison). (Standard-Bearer: `ios-swift/Notelayer/Notelayer/Views/ProfileSettingsView.swift`)
  - [ ] 🟩 Audit current appearance UI for deviations from `ui-consistency.md` and note line-count impact. (Deviation: half-height Themes sheet uses custom layout instead of List to avoid scroll; estimated +120 lines for custom tiles.)
...
- [ ] 🟩 **Step 7: Implementation Plan & Test Strategy**
  ...
  - [ ] 🟩 Define UI verification steps (live preview, persistence, migration correctness). (Manual: apply preset → open customize → adjust wallpaper/accent/surface/intensity → save theme → relaunch to confirm persistence → verify migration behavior.)
